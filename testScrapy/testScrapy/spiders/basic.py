...
from testScrapy.items import Test1Item
...
class BasicSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        self.log("content: %s" % response.xpath(
            '//*[@id="Cnt-Main-Article-QQ"]//text()'
        ).extract())   # 因为是动态的，所以是提取不到这个东西的。破解js的话，scrapy 需要怎么操作。

        item['images'] =response.xpath(
            '//*[@id="Cnt-Main-Article-QQ"]//img'
        ).extract()   # 因为是动态的，所以是提取不到这个东西的。破解js的话，scrapy 需要怎么操作。

        item['image_urls'] = response.xpath('//*[@id="Cnt-Main-Article-QQ"]//img/@src').extract() 
        print(type(response.xpath('//*[@id="Cnt-Main-Article-QQ"]//img/@src').extract()))
        self.log("img:http:%s" % response.xpath(
                '//*[@id="Cnt-Main-Article-QQ"]//img/@src'
        ).extract())  # 因为是动态的，所以是提取不到这个东西的。破解js的话，scrapy 需要怎么操作。
